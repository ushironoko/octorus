...
# PR 選択後に AI Rally を開始
or --ai-rally
```

`--repo` と `--pr` はどちらも省略可能です:
- `--repo` を省略すると、カレントディレクトリの git リモートからリポジトリを自動検出
- `--pr` を省略すると、PR 一覧が表示され対話的に選択可能

### オプション

| オプション | 説明 |
|--------|-------------|
| `-r, --repo <REPO>` | リポジトリ名（例: "owner/repo"）。省略時は自動検出 |
| `-p, --pr <PR>` | プルリクエスト番号。省略時は PR 一覧から選択 |
| `--refresh` | キャッシュを無視して強制更新 |
| `--cache-ttl <SECS>` | キャッシュ TTL（秒）（デフォルト: 300） |
| `--ai-rally` | PR 選択後に AI Rally を開始 |
...
`--pr` を省略すると表示されます。レビューする PR を一覧から選択します。
...
**機能:**
- 無限スクロール: 一覧の下部に近づくと自動的に追加の PR を読み込み
- ファイル一覧画面や Diff 画面で `q` を押すと PR 一覧に戻る（PR 一覧から開いた場合）
