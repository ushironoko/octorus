...
- ステージング・コミット・履歴閲覧・reset --softを統合したgit ops view
...
| `-i, --issue [NUMBER]` | Issue 一覧を開く。番号を指定すると直接その Issue を表示 |
| `--local` | GitHub 取得をせず、`HEAD` との差分を表示 |
| `--auto-focus` | ローカルモード時に差分更新があったファイルへ自動フォーカス |
| `--git-ops` | 起動時に Git Ops 画面を直接開く |
...
ステージング・コミット・コミット履歴閲覧・reset --soft を統合した画面です。左ペインはファイルツリー（70%）とコミット履歴（30%）に分割され、右ペインに diff プレビューが表示されます。

破壊的操作（discard, undo, reset）は実行される git コマンドを表示した Y/n 確認プロンプトが表示されます。コミットペインのタイトルにはローカルがリモートより先行している場合 `↑N` が表示されます。

CLI から `--git-ops` フラグで直接開くこともできます。
...
| `j` / `↓` | 下に移動 |
| `k` / `↑` | 上に移動 |
| `Space` | ファイル/ディレクトリの stage/unstage |
| `s` | 全ファイルを stage |
| `d` | 変更を破棄（Y/n 確認） |
| `c` | コミット（エディタ起動） |
| `u` | 直前の操作を undo（Y/n 確認） |
| `R` | ステータスをリフレッシュ |
| `P` | origin に push（ローディングスピナー表示） |
| `Enter` | ディレクトリ展開/折畳、またはdiffにフォーカス |
| `Tab` | コミットペインに切替 |
| `l` / `→` | diff ペインにフォーカス |
| `q` / `Esc` | Git ops を閉じる |
...
| `g` | 先頭のコミットにジャンプ |
| `G` | 末尾のコミットにジャンプ |
| `u` | 選択コミットへ reset --soft（ローカルモードのみ、Y/n 確認） |
| `Tab` | ツリーペインに切替 |
| `Enter` / `l` / `→` | diff ペインにフォーカス（コミット diff） |
| `q` / `Esc` | Git ops を閉じる |
...
コミットは無限スクロールで読み込まれます。diff はバックグラウンドでプリフェッチされ、高速なナビゲーションを実現します。
...
| `G` | 末尾にジャンプ |
| `Enter` | Issue 詳細を表示 |
| `o` | フィルタ: Open Issue のみ |
| `c` | フィルタ: Closed Issue のみ |
| `a` | フィルタ: すべての Issue |
| `O` | Issue をブラウザで開く |
| `R` | Issue 一覧をリフレッシュ |
| `Space /` | キーワードフィルタ |
